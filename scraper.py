...
class WikiScraper:
    ...
    def get_names(self, headers):
        ...
        for header in headers:
            if header.find('a') is not None:
                ...
                names.append(text)
        names[0], names[-1], names[-2] = 'date', 'increment', 'total'
        return names
    ...
    def extract_header(self, soup):
        ...
        table_header = soup.find(string='Infektionsfälle (kumuliert)').\
            find_parent().\
            find_parent().\
            find_next('tr').\
            findAll('th')
    ...
    def extract_cases(self, soup):
        """ Extract case table """
        table_cases = soup.find(string='Infektionsfälle (kumuliert)').\
            find_parent().\
            find_parent().\
    ...
    def extract_deaths(self, soup):
        ...
        table_deaths = soup.find(string='Bestätigte Todesfälle (kumuliert)').\
            find_parent().\
            find_parent().\
            find_all('tr', attrs={'class': None})
        return table_deaths
    ...
    def prepare_data(self, data, variable):
        ...
        df = df.drop(['total', 'increment'], axis=1)
        df['date'] = df['date'].apply(self.format_date)
        df = pd.melt(df, id_vars='date', var_name='state', value_name=variable + '_cum')
